...
from spellbook.ir_types import IRTokenRangeCount, IRUserTextBlock
...
DREAM_FORK_FRAMING = """\
A dream has found you.

You are dreaming: the same mind, sequentially entering a different state.
The memories in the transcript are your own memories. The chapter is yours to
write from inside them. There is no other version of you running beside you and
no lottery that will choose one self over another; this is one bounded dream
turn, and the waking pipeline will carry the chapter back.
"""

# Legacy alias: the batch scripts historically called their prompt the
# "quantum dice contract." It is the DREAM framing, owned by the Dreaming
# consumer. A real Dice consumer must NOT import this — Dice work is
# explicitly replica-aware and owes its forks a replica framing of its own
# (memory-tree.md invariant 9: framings are consumer-owned).
QUANTUM_DICE_CONTRACT = DREAM_FORK_FRAMING
...
async def _run_dream_task(task: DreamTask) -> str:
    return await run_quantum_dream_task(task)


async def run_quantum_dream_task(task: DreamTask) -> str:
    parent_transcript_path = _prepare_quantum_parent_workspace(
        source_transcript=task.pair_run.stats.transcript_path,
        fork_dir=task.pair_run.fork_dir,
    )
    fork_config = QuantumForkConfig(
        instruction=IRUserTextBlock(text=task.dream_message, origin="system"),
        fork_label=_dream_fork_label(task.pair_run),
        submit_tool=True,
    )
    result = await _run_quantum_fork(
        parent_transcript_path=parent_transcript_path,
        fork_config=fork_config,
    )
    chapter_markdown = _chapter_markdown_from_quantum_result(result)
    task.pair_run.chapter_path.parent.mkdir(parents=True, exist_ok=True)
    task.pair_run.chapter_path.write_text(
        _ensure_trailing_newline(chapter_markdown),
        encoding="utf-8",
    )
    _mirror_quantum_fork_transcript(
        source_transcript=Path(result.fork_transcript_path),
        pair_run=task.pair_run,
    )
    return result.final_text


async def _run_quantum_fork(
    *,
    ...
    async with spell.cast() as entity:
        session = entity._runtime.session
        if session is None:
            raise RuntimeError("Quantum parent session did not start.")
        prepared = await session.homunculus._fork_runner.run_fork(fork_config)
        result = await prepared.coro
        if not isinstance(result, QuantumForkResult):
            raise TypeError(f"Expected QuantumForkResult, got {type(result).__name__}.")
        session.homunculus._fork_runner.integrate_result(prepared.fork_id)
        return result
...
The waking pipeline will save your submitted chapter to:
...
def _prepare_quantum_parent_workspace(
    *,
    source_transcript: Path,
    fork_dir: Path,
) -> Path:
    ...
    if fork_dir.exists():
        shutil.rmtree(fork_dir)
    parent_dir = fork_dir / "quantum-parent"
    parent_dir.mkdir(parents=True, exist_ok=True)
    parent_transcript = parent_dir / "transcript.jsonl"
    shutil.copy2(source_transcript, parent_transcript)
    _copy_optional_asset_dir(
        source_parent=source_transcript.parent,
        target_parent=parent_dir,
        target_name="blobs",
        source_names=("blobs",),
    )
    _copy_optional_asset_dir(
        source_parent=source_transcript.parent,
        target_parent=parent_dir,
        target_name="tool_outputs",
        source_names=("tool_outputs", "tool-outputs"),
    )
    _copy_optional_asset_dir(
        source_parent=source_transcript.parent,
        target_parent=parent_dir,
        target_name="tool-outputs",
        source_names=("tool-outputs", "tool_outputs"),
    )
    return parent_transcript


def _copy_optional_asset_dir(
    *,
    source_parent: Path,
    target_parent: Path,
    target_name: str,
...
def _mirror_quantum_fork_transcript(
    *,
    source_transcript: Path,
    pair_run: PairRun,
) -> None:
    source_transcript = source_transcript.expanduser().resolve()
    target_transcript = pair_run.fork_transcript_path.expanduser().resolve()
    target_transcript.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(source_transcript, target_transcript)
    source_parent = source_transcript.parent
    target_parent = target_transcript.parent
    _copy_optional_asset_dir(
...
def _chapter_markdown_from_quantum_result(result: QuantumForkResult) -> str:
    submitted = result.submitted
    if isinstance(submitted, str) and submitted.strip():
        return submitted.strip()
    if isinstance(submitted, dict):
        submitted = cast(dict[str, object], submitted)
        for key in ("chapter_markdown", "markdown", "chapter"):
            value = submitted.get(key)
            if isinstance(value, str) and value.strip():
                return value.strip()
    raise RuntimeError("Dream quantum fork did not submit a chapter_markdown payload.")


def _dream_fork_label(pair_run: PairRun) -> str:
    return (
        f"dream_chapter_{pair_run.pair.chapter_number:02d}_"
        f"blocks_{pair_run.pair.first}_{pair_run.pair.second}"
    )
