"""Repair correspondent semantic block boundaries that split tool pairs.

This is a guarded transcript surgery script for the June 2026 correspondent
boundary incident. It edits only six known integers inside block_detection
records' completed ranges:

  block 0  range 0db0258e...: end_block     51 -> 52
  block 1  range 40129e53...: start_block   52 -> 53
  block 1  range 40129e53...: end_block     71 -> 72
  block 2  range 44d4a4ad...: start_block   72 -> 73
  block 2  range 44d4a4ad...: end_block     80 -> 81
  block 3  range 6c31d69c...: start_block   81 -> 82
...
Usage:
  uv run python scripts/repair_correspondent_boundaries.py --dry-run
  uv run python scripts/repair_correspondent_boundaries.py --execute
"""
...
TRANSCRIPT = Path(
    "/home/rheaton64/.chorus/spellbook/sessions/correspondent/transcript.jsonl"
)
...
EDITS = [
    BoundaryEdit(
        "block_range_0db0258e617e422889735f1a8a09f5fb",
        "end_block",
        51,
        52,
    ),
    BoundaryEdit(
        "block_range_40129e53927049daa532a6f87a5be6be",
        "start_block",
        52,
        53,
    ),
    ...
    BoundaryEdit(
        "block_range_44d4a4ad69794b37874b6e0a41c32aac",
        "start_block",
        72,
        73,
    ),
    ...
    BoundaryEdit(
        "block_range_6c31d69cd4e74fa0a18459f07e987154",
        "start_block",
        81,
        82,
    ),
...
def _copy_for_dry_run(source: Path) -> Path:
    ...
    target = WORKDIR / "correspondent_boundary_dryrun.jsonl"
...
def _backup(source: Path) -> Path:
    WORKDIR.mkdir(parents=True, exist_ok=True)
    backup = (
        WORKDIR / f"correspondent_boundary_backup_{datetime.now():%Y%m%d_%H%M%S}.jsonl"
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Repair correspondent tool_call/tool_result semantic boundary splits."
    )
